...
async function HomeContent() {
  ...
  if (onboarding && requiresOnboardingRedirect(onboarding.onboardingStatus)) {
    redirect(routes.cleanerApplication);
  }
  ...
  return (
    <>
      <ProHeader
        title={`Hi, ${greeting}`}
        description="Jobs, earnings, and MaidLinx Verified progress."
      />
      ...
      {!canWork ? (
        ...
        >
          <div>
            <p className="font-medium text-navy">Finish Verified pipeline</p>
            <p className="text-sm text-ink-muted">{progress.percent}% onboarding checklist</p>
          </div>
...
export default function CleanerHomePage() {
  return (
    <div className="mx-auto max-w-lg px-4 py-5 sm:max-w-xl lg:max-w-3xl lg:px-8 lg:py-8">
